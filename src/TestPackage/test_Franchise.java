...
public class test_Franchise {
	...
	void testChiffreAffaireFranchise() {
		...
		Franchise f0 = new Franchise(x0, y0, y0, 0.0);
		Franchise f1 = new Franchise(x1, y1, y1, 0.0);
		Franchise f2 = new Franchise(x2, y2, y2, 0.0);
		Franchise f3 = new Franchise(x3, y3, y3, 0.0);
		
		// Arrange - on ajoute les serveurs aux restaurants
		for(Restaurant r : f0.get_aoRestaurants()) {
			for(int i = 0; i<y0; i++) {
				Serveur s = r.get_aoServeurs().get(i); // on prend un servuer serveur
				r.assignerTable(r.get_aoServeurs().get(i), r.get_aoTables().get(i)); // on lui assigneune table
				s.prendCommande(new Commande(dMontantCommande, false),r.get_aoTables().get(i) ); // il prend une commande
			}
		}
		
		for(Restaurant r : f1.get_aoRestaurants()) {
			for(int i = 0; i<y1; i++) {
				Serveur s = r.get_aoServeurs().get(i);
